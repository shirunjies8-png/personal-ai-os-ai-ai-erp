...
const MODULES = [
  ...
  { id: 'files', name: '文件管理中心', icon: 'folder', group: '数据管理' },
  ...
  { id: 'toolcenter', name: 'Tool Center', icon: 'apps', group: '系统中心' },
...
const UI = {
  ...
  home() {
    ...
    return `${this.pageHead('Industrial AI OS', '面向制造业与企业办公的 AI 操作系统', `<button class="secondary-btn" data-action="demo-flow">${icon('play')}快捷体验</button><button class="secondary-btn" data-action="demo-load">${icon('download')}加载示例数据</button><button class="secondary-btn" data-action="demo-reset">${icon('trash')}清空测试数据</button><button class="primary-btn" data-route="templates">${icon('book')}打开模板中心</button>`)}
    <section class="panel home-greeting"><div><p class="eyebrow">${new Date().toLocaleDateString('zh-CN', { month: 'long', day: 'numeric', weekday: 'long' })}</p><h2>Industrial AI OS</h2><p>Enterprise AI Platform</p><div class="hero-meta"><span class="status-pill">Version：v1.3 实用版</span><span class="status-pill success">Project Status：可实际操作</span></div></div><div class="ai-mode"><span class="mode-orb">${icon('sparkles')}</span><span><b>${accessModeLabel}</b><small>${s.settings.accessMode === 'local' ? 'Local Only / Mock 兜底' : s.settings.model}</small></span></div></section>
    <section class="panel"><div class="panel-head"><div><h3>访问地址</h3></div><span class="badge">可访问地址显示区</span></div><div class="panel-body"><div class="address-grid"><div class="address-card"><b>当前访问地址</b><small>${Utils.escape(window.location.origin || '本地运行中')}</small></div><div class="address-card"><b>本地地址</b><small>http://127.0.0.1:8080</small></div><div class="address-card"><b>GitHub Pages 地址</b><small>${Utils.escape(githubAddress)}</small></div><div class="address-card"><b>项目状态</b><small>v1.3 实用版</small></div><div class="address-card"><b>AI模式</b><small>${accessModeLabel}</small></div><div class="address-card"><b>数据模式</b><small>${accessModeLabel === 'Mock 本地演示' ? 'Local Only' : accessModeLabel === 'Hybrid 混合模式' ? 'Hybrid' : 'Remote AI'}</small></div></div></div></section>
  ...
  chat() {
    ...
    const chatModeLabel = displayMode
      ? 'GitHub Pages 展示模式 · Mock AI · DeepSeek 未连接'
      : Store.state.settings.accessMode === 'local'
        ? '本地模式 · Mock AI'
        : Store.state.settings.accessMode === 'api'
          ? 'API模式 · 真实 AI 可用'
          : '云端模式 · Hybrid';
    const composerHint = displayMode
      ? '当前为 GitHub Pages 展示模式，真实 AI 后端未连接。可以继续提问、挂载文件和查看历史。'
      : '支持文件问答和上下文记忆';
    ...
    <section class="panel chat-layout"><aside class="chat-history"><div class="chat-history-head"><input class="input" id="chatSearch" placeholder="搜索历史聊天" value="${Utils.escape(App.temp.chatSearch || '')}" style="margin-bottom:10px"><button class="primary-btn" style="width:100%" data-action="chat-new">${icon('plus')}新建聊天</button></div><div class="chat-history-list">${chats.length ? chats.map(chat => `<button class="chat-session ${active?.id === chat.id ? 'active' : ''}" data-action="chat-open" data-id="${chat.id}"><span>${icon('message')}</span><div><b>${Utils.escape(chat.title)}</b><small>${chat.messages.length} 条消息 · ${(chat.files || []).length} 个文件</small></div></button>`).join('') : `<div class="empty-result"><span>${icon('message')}</span><b>暂无历史聊天</b><small>点击上方按钮创建会话</small></div>`}</div></aside><div class="chat-main"><div class="chat-toolbar"><div><b>${Utils.escape(active?.title || '新对话')}</b><small>${Utils.escape(chatModeLabel)} · 自动保存${active?.files?.length ? ` · 已挂载 ${active.files.length} 个文件` : ''}</small></div><span class="status-pill success">${icon('shield')} 隐私模式</span></div><div class="chat-messages" id="chatMessages">${active?.messages.length ? active.messages.map(message => `<div class="message ${message.role === 'user' ? 'user' : ''}"><span class="message-avatar">${message.role === 'user' ? '我' : 'AI'}</span><div class="message-body"><div class="message-meta"><span>${message.role === 'user' ? '你' : 'Personal AI'}</span><time>${Utils.formatDate(message.time, true)}</time>${message.role === 'assistant' ? `<button class="copy-message" data-action="chat-copy" data-text="${encodeURIComponent(message.content)}">复制</button>` : ''}</div><div class="message-content">${Utils.escape(message.content)}</div></div></div>`).join('') : `<div class="empty-result"><span>${icon('sparkles')}</span><b>开始企业办公问答</b><small>可直接提问，也可先挂载文件再问答</small></div>`}</div><form class="chat-composer" data-form="chat"><div class="composer-box"><textarea id="chatInput" placeholder="输入问题，支持 /总结、/整理、/生成日报 等快捷命令"></textarea><div class="composer-actions"><span>${Utils.escape(composerHint)}</span><button class="primary-btn" type="submit">${icon('send')}发送</button></div></div></form></div></section>`;
  },
  assistant() {
  ...
  pdf() {
    ...
    return `${this.pageHead('PDF助手', '上传 PDF、自动解析、AI总结、AI翻译、AI问答与结果导出；扫描件会自动 OCR。', `<label class="primary-btn">${icon('upload')}上传PDF<input type="file" data-input="pdf-files" accept=".pdf,application/pdf" multiple hidden></label><button class="secondary-btn" data-action="pdf-sample">${icon('pdf')}加载示例PDF</button><button class="secondary-btn" data-action="pdf-ocr">${icon('scan')}转入OCR识别</button><button class="secondary-btn" data-action="pdf-export">${icon('download')}导出结果</button>`)}<div class="workbench"><div class="stack"><section class="panel"><div class="panel-head"><div><h3>PDF 文件</h3><span class="badge">${infos.length}</span></div><span class="status-pill ${p.scanMode === 'scan' ? 'warning' : 'success'}">${p.files?.length ? (p.scanMode === 'ocr' ? '扫描件 OCR' : '读取成功') : '等待上传'}</span></div><div class="panel-body">${this.upload('pdf-files', '选择 PDF 文件', '支持一个或多个 .pdf 文件', '.pdf', true)}${infos.length ? `<div class="kb-list">${infos.map(info => `<article class="kb-item"><span>${icon('pdf')}</span><div><b>${Utils.escape(info.name)}</b><p>${Utils.formatBytes(info.size)} · ${info.pages} 页</p><small>${Utils.escape(info.status)}</small></div></article>`).join('')}</div>` : this.result('', '尚未上传 PDF 文件。')} ${p.scanMode === 'scan' ? `<div class="privacy-note">${icon('scan')}<span>该 PDF 可能是复杂矢量图、扫描质量较低或加密文件，请尝试上传截图、转换为图片，或手动粘贴文本。</span></div>` : ''}<div class="privacy-note">${icon('shield')}<span>当前如果启用远程 AI，输入内容可能发送至第三方 AI 接口。请勿上传企业机密、真实客户隐私、财务数据、合同原件或未脱敏文件。</span></div><div class="button-row"><button class="secondary-btn" data-action="settings-dev-toggle">${icon('shield')}先脱敏再发送 AI</button><button class="secondary-btn" data-action="settings-dev-toggle">${icon('check')}我确认使用远程 AI</button><button class="secondary-btn" data-action="settings-api-toggle">${icon('save')}仅本地 Mock 分析</button></div></div></section><section class="panel"><div class="panel-head"><h3>PDF 操作</h3></div><div class="panel-body"><div class="button-row"><button class="primary-btn" data-action="pdf-summary">${icon('sparkles')}AI总结PDF</button><button class="secondary-btn" data-action="pdf-translate">${icon('refresh')}AI翻译</button><button class="secondary-btn" data-action="pdf-extract">${icon('fileText')}读取文字</button><button class="secondary-btn" data-action="pdf-table">${icon('table')}提取表格</button><button class="secondary-btn" data-action="pdf-split">${icon('split')}拆分</button><button class="secondary-btn" data-action="pdf-merge">${icon('merge')}合并</button><button class="secondary-btn" data-action="pdf-word">${icon('download')}转Word</button></div><div class="field"><label>PDF问答</label><textarea class="textarea" id="pdfQuestion" placeholder="输入关于当前 PDF 的问题">${Utils.escape(p.qaQuestion || '')}</textarea><button class="primary-btn" data-action="pdf-qa">${icon('message')}提问</button></div></div></section></div><div class="stack"><section class="panel"><div class="panel-head"><div><h3>读取文字预览</h3></div><span class="badge">${(p.extracted || '').length}</span></div><div class="panel-body"><textarea class="textarea tall" readonly placeholder="上传后显示读取到的 PDF 文字">${Utils.escape((p.extracted || '').slice(0, 16000))}</textarea></div></section><section class="panel"><div class="panel-head"><h3>处理结果</h3><button class="ghost-btn compact" data-action="copy-result" data-source="pdfResult">${icon('copy')}复制</button></div><div class="panel-body" id="pdfResult">${this.result(p.result || p.qaAnswer || p.analysis || '', '等待 PDF 读取、总结、翻译或问答结果。', true)}</div></section></div></div>`;
  },
  pdfLegacy() {
    const p = App.temp.pdf;
  ...
  files() {
    ...
    return `${this.pageHead('文件管理中心', '上传后自动联动到 Excel / Word / PDF / 图片助手，并支持搜索、排序、重命名、删除、下载、收藏', `<label class="primary-btn">${icon('upload')}上传文件<input type="file" data-input="file-center" multiple hidden></label>`)}<div class="workbench"><div class="stack"><section class="panel"><div class="panel-head"><div><h3>最近打开 / 最近上传</h3><span class="badge">${recentFiles.length}</span></div></div><div class="panel-body">${recentFiles.length ? `<div class="kb-list">${recentFiles.map(file => `<article class="kb-item"><span>${icon(file.category === '表格' ? 'table' : file.category === 'PDF' ? 'pdf' : file.category === '图片' ? 'image' : 'fileText')}</span><div><b>${Utils.escape(file.name)}</b><p>${file.category} · 打开 ${file.openCount || 0} 次 · 上传于 ${Utils.formatDate(file.uploadedAt, true)}</p><small>${file.favorite ? '已收藏' : '未收藏'}</small></div><button class="icon-btn" data-action="file-open" data-id="${file.id}">${icon('eye')}</button></article>`).join('')}</div>` : this.result('', '打开文件后会在这里保留最近记录')}</div></section></div><section class="panel"><div class="panel-head"><div><h3>本地文件</h3><span class="badge">${files.length}</span></div><span class="status-pill success">IndexedDB</span></div><div class="panel-body"><div class="file-toolbar"><div class="search-box">${icon('search')}<input class="input" id="fileSearch" placeholder="搜索文件名" value="${Utils.escape(App.temp.fileSearch)}"></div><select class="select" id="fileCategory" style="width:120px"><option>全部</option>${['表格', '文档', 'PDF', '图片', '代码', '其他'].map(item => `<option ${cat === item ? 'selected' : ''}>${item}</option>`).join('')}</select><select class="select" id="fileSort" style="width:150px"><option value="updated_desc" ${sort === 'updated_desc' ? 'selected' : ''}>最近打开</option><option value="uploaded_desc" ${sort === 'uploaded_desc' ? 'selected' : ''}>最近上传</option><option value="name_asc" ${sort === 'name_asc' ? 'selected' : ''}>名称排序</option><option value="favorite" ${sort === 'favorite' ? 'selected' : ''}>收藏优先</option></select></div>${files.length ? `<div class="table-wrap"><table class="data-table file-table"><thead><tr><th>文件</th><th>分类</th><th>大小</th><th>最近时间</th><th>操作</th></tr></thead><tbody>${files.map(file => `<tr><td><div class="file-name"><span class="file-type-icon">${icon(file.category === '图片' ? 'image' : file.category === '表格' ? 'table' : file.category === 'PDF' ? 'pdf' : 'fileText')}</span><span><b>${Utils.escape(file.name)}</b><small>${Utils.escape(file.type || '未知类型')} · 打开 ${file.openCount || 0} 次</small></span></div></td><td>${file.category}</td><td>${Utils.formatBytes(file.size)}</td><td>${Utils.formatDate(file.updatedAt, true)}</td><td><div class="table-actions"><button class="icon-btn favorite ${file.favorite ? 'active' : ''}" data-action="file-favorite" data-id="${file.id}" title="收藏">${icon('star')}</button><button class="icon-btn" data-action="file-open" data-id="${file.id}" title="打开">${icon('eye')}</button><button class="icon-btn" data-action="file-rename" data-id="${file.id}" title="重命名">${icon('pen')}</button><button class="icon-btn" data-action="file-download" data-id="${file.id}" title="下载">${icon('download')}</button><button class="icon-btn" data-action="file-delete" data-id="${file.id}" title="删除">${icon('trash')}</button></div></td></tr>`).join('')}</tbody></table></div>` : this.result('', q ? '没有匹配的文件' : '上传文件后开始管理')}</div></section></div>`;
  },
  taskcenter() {
    const items = (Store.state.taskRecords || []).slice(0, 50);
    const selected = items.find(item => item.id === App.temp.taskSelectedId) || items[0];
    const routeMap = { pdf: 'pdf', ocr: 'ocr', excel: 'excel', word: 'word', ppt: 'ppt', sql: 'sql', chat: 'chat' };
    const statusClass = status => ['完成', 'success'].includes(status) ? 'success' : ['失败', 'failed', 'timeout', 'cancelled'].includes(status) ? 'warning' : 'amber';
    return `${this.pageHead('任务中心', '统一管理 PDF / OCR / Excel / AI 聊天与 Agent Runtime 任务，支持查看详情、取消和重试。', `<button class="secondary-btn" data-action="task-refresh">${icon('refresh')}刷新任务</button><button class="primary-btn" data-action="task-refresh">${icon('check')}同步任务队列</button>`)}<div class="workbench"><div class="stack"><section class="panel"><div class="panel-head"><div><h3>任务列表</h3></div><span class="badge">${items.length}</span></div><div class="panel-body">${items.length ? `<div class="kb-list">${items.map(item => `<article class="kb-item" style="align-items:center"><span>${icon(item.module === 'pdf' ? 'pdf' : item.module === 'ocr' ? 'scan' : item.module === 'excel' ? 'table' : item.module === 'chat' ? 'message' : 'bot')}</span><div><b>${Utils.escape(item.type || item.fileName || '任务')}</b><p>${Utils.escape(item.fileName || item.summary || '')}</p><small>${Utils.escape(item.module || 'task')} · ${Utils.formatDate(item.time || item.createdAt || Date.now(), true)}</small></div><span class="status-pill ${statusClass(item.status)}">${Utils.escape(item.status || '处理中')}</span><button class="icon-btn" data-action="task-open" data-id="${item.id}" title="查看摘要">${icon('eye')}</button><button class="icon-btn" data-action="task-open-result" data-id="${item.id}" data-route="${routeMap[item.module] || 'taskcenter'}" title="跳转结果页">${icon('search')}</button>${item.runtime ? `<button class="icon-btn" data-action="task-cancel" data-id="${item.id}" title="取消任务">${icon('x')}</button><button class="icon-btn" data-action="task-retry" data-id="${item.id}" title="重试任务">${icon('refresh')}</button>` : ''}</article>`).join('')}</div>` : this.result('', '暂无任务记录，先执行 PDF / OCR / Excel / AI 相关流程')}</div></section></div><div class="stack"><section class="panel"><div class="panel-head"><div><h3>结果摘要</h3></div><span class="status-pill ${selected ? statusClass(selected.status) : ''}">${Utils.escape(selected?.status || '未选择')}</span></div><div class="panel-body">${selected ? `<div class="result-box large">${Utils.textToHtml(`任务类型：${selected.type || '未知'}\n来源模块：${selected.module || '未知'}\n文件名：${selected.fileName || '无'}\n状态：${selected.status || '未知'}\n时间：${Utils.formatDate(selected.time || selected.createdAt || Date.now(), true)}\n\n摘要：\n${selected.summary || '无摘要'}\n\n结果：\n${selected.result || '无结果'}`)}</div>` : this.result('', '点击左侧任务查看结果摘要', true)}</div></section><section class="panel"><div class="panel-head"><div><h3>任务来源</h3></div></div><div class="panel-body">${this.result('PDF / OCR / Excel 的处理、总结、翻译、问答、导出，以及 Agent Runtime 的创建、执行、审批、取消和重试都会自动记录为任务。', '任务中心会显示真实处理记录', true)}</div></section></div></div>`;
  },
  toolcenter() {
    const items = Store.state.toolCatalog || [];
    const selected = items.find(item => item.toolName === App.temp.toolSelectedName) || items[0];
    const ws = App.getWorkspace('toolcenter');
    return `${this.pageHead('Tool Center', '统一管理 8 个可执行工具，支持参数校验、权限检查、超时、重试和真实执行。', `<button class="secondary-btn" data-action="toolcenter-refresh">${icon('refresh')}刷新工具</button><button class="primary-btn" data-action="toolcenter-run">${icon('play')}执行工具</button>`)}<div class="workbench"><div class="stack"><section class="panel"><div class="panel-head"><div><h3>工具清单</h3></div><span class="badge">${items.length}</span></div><div class="panel-body">${items.length ? `<div class="kb-list">${items.map(item => `<article class="kb-item" style="cursor:pointer" data-action="toolcenter-select" data-id="${item.toolName}"><span>${icon('apps')}</span><div><b>${Utils.escape(item.toolName)}</b><p>${Utils.escape(item.description || '')}</p><small>${Utils.escape(item.permissionLevel || 'viewer')} · ${Number(item.timeoutMs || 0)} ms</small></div></article>`).join('')}</div>` : this.result('', '登录后会从后端读取 Tool Center', true)}</div></section></div><div class="stack"><section class="panel"><div class="panel-head"><div><h3>${Utils.escape(selected?.toolName || '选择工具')}</h3></div><span class="status-pill">${Utils.escape(selected?.permissionLevel || 'viewer')}</span></div><div class="panel-body"><div class="field"><label>执行参数（JSON）</label><textarea class="textarea tall" id="toolcenterInput" placeholder='{"text":"示例内容","filename":"demo.csv"}'>${Utils.escape(ws.prompt || '')}</textarea></div><div class="privacy-note">${icon('shield')}<span>高风险工具会受到权限和审批约束；viewer 只能查看，operator 可执行普通工具，admin 才可执行高风险工具。</span></div><div class="button-row"><button class="secondary-btn" data-action="toolcenter-refresh">${icon('refresh')}刷新</button><button class="primary-btn" data-action="toolcenter-run">${icon('play')}执行工具</button></div></div></section><section class="panel"><div class="panel-head"><div><h3>执行结果</h3></div></div><div class="panel-body">${this.result(ws.result || '', '工具执行结果会显示在这里', true)}</div></section></div></div>`;
  },
  downloadcenter() {
    const items = (Store.state.downloadRecords || []).slice(0, 50);
    const selected = items.find(item => item.id === App.temp.downloadSelectedId) || items[0];
    return `${this.pageHead('下载中心', '集中显示所有导出文件，支持下载、查看来源模块、生成时间和反馈提示。', `<button class="secondary-btn" data-action="download-refresh">${icon('refresh')}刷新下载</button><button class="primary-btn" data-action="download-refresh">${icon('check')}下载记录已联动</button>`)}<div class="workbench"><div class="stack"><section class="panel"><div class="panel-head"><div><h3>下载列表</h3></div><span class="badge">${items.length}</span></div><div class="panel-body">${items.length ? `<div class="kb-list">${items.map(item => `<article class="kb-item" style="align-items:center"><span>${icon(item.fileType === 'pdf' ? 'pdf' : item.fileType === 'xlsx' ? 'table' : item.fileType === 'docx' ? 'fileText' : 'download')}</span><div><b>${Utils.escape(item.filename || '未命名文件')}</b><p>${Utils.escape(item.sourceModule || '未知模块')} · ${Utils.formatDate(item.time || item.createdAt || Date.now(), true)}</p><small>${Utils.escape(item.status || '已生成')} · ${Utils.escape(item.summary || '')}</small></div><span class="status-pill success">${Utils.escape(item.fileType || 'file')}</span><button class="icon-btn" data-action="download-open" data-id="${item.id}" title="查看详情">${icon('eye')}</button><button class="icon-btn" data-action="download-run" data-id="${item.id}" title="下载文件">${icon('download')}</button></article>`).join('')}</div>` : this.result('', '暂无下载记录，执行 PDF / OCR / Excel 导出后会自动进入这里')}</div></section></div><div class="stack"><section class="panel"><div class="panel-head"><div><h3>下载详情</h3></div><span class="status-pill success">${selected ? Utils.escape(selected.status || '已生成') : '未选择'}</span></div><div class="panel-body">${selected ? `<div class="result-box large">${Utils.textToHtml(`文件名：${selected.filename || '无'}\n来源模块：${selected.sourceModule || '无'}\n来源文件：${selected.sourceName || '无'}\n文件类型：${selected.fileType || '无'}\n生成时间：${Utils.formatDate(selected.time || selected.createdAt || Date.now(), true)}\n\n说明：${selected.summary || '无'}\n`)}</div><div class="button-row"><button class="secondary-btn" data-action="download-open" data-id="${selected.id}">${icon('eye')}查看详情</button><button class="primary-btn" data-action="download-run" data-id="${selected.id}">${icon('download')}下载</button></div>` : this.result('', '点击左侧下载记录查看详情', true)}</div></section><section class="panel"><div class="panel-head"><div><h3>下载来源说明</h3></div></div><div class="panel-body">${this.result('PDF 总结、OCR TXT、Excel 导出、Word/PDF 转换结果都会自动进入下载中心。', '下载中心会保留本地下载记录', true)}</div></section></div></div>`;
  },
  approval() {
    const items = Store.state.agentApprovals || [];
    const selected = items.find(item => item.id === App.temp.approvalSelectedId) || items[0];
    return `${this.pageHead('Human Approval', '高风险动作会进入 waiting_human，审批通过后继续执行，拒绝后任务终止。', `<button class="secondary-btn" data-action="approval-refresh">${icon('refresh')}刷新审批</button>${selected ? `<button class="secondary-btn" data-action="approval-reject" data-id="${selected.taskId}">${icon('x')}拒绝</button><button class="primary-btn" data-action="approval-approve" data-id="${selected.taskId}">${icon('check')}批准</button>` : ''}`)}<div class="workbench"><div class="stack"><section class="panel"><div class="panel-head"><div><h3>待审批请求</h3></div><span class="badge">${items.length}</span></div><div class="panel-body">${items.length ? `<div class="kb-list">${items.map(item => `<article class="kb-item" data-action="approval-select" data-id="${item.id}" style="cursor:pointer"><span>${icon('shield')}</span><div><b>${Utils.escape(item.actionLabel || item.toolName)}</b><p>${Utils.escape(item.reason || '')}</p><small>${Utils.escape(item.taskId || '')} · ${Utils.formatDate(item.createdAt || Date.now(), true)}</small></div><span class="status-pill amber">${Utils.escape(item.status || 'pending')}</span></article>`).join('')}</div>` : this.result('', '当前没有待审批任务', true)}</div></section></div><div class="stack"><section class="panel"><div class="panel-head"><div><h3>审批详情</h3></div></div><div class="panel-body">${selected ? `<div class="result-box large">${Utils.textToHtml(`任务ID：${selected.taskId || ''}\n工具：${selected.toolName || ''}\n动作：${selected.actionLabel || ''}\n状态：${selected.status || ''}\n原因：${selected.reason || ''}`)}</div><div class="button-row"><button class="secondary-btn" data-action="approval-reject" data-id="${selected.taskId}">${icon('x')}拒绝</button><button class="primary-btn" data-action="approval-approve" data-id="${selected.taskId}">${icon('check')}批准并继续</button></div>` : this.result('', '点击左侧审批项查看详情', true)}</div></section></div></div>`;
  },
  orders() {
  ...
  aistatus() {
    ...
    const items = [
      ['DeepSeek', displayMode ? '🟡 未连接' : Store.state.settings.apiEnabled ? '🟢 正常' : '🟡 未连接'],
      ['API状态', displayMode ? '🟡 展示模式' : Store.state.settings.apiUrl ? '🟢 正常' : '🟡 未配置'],
      ['AI Gateway', `${displayMode ? '🟡' : gateway.state === 'online' ? '🟢' : gateway.state === 'fallback' ? '🟡' : '🟡'} ${displayMode ? '当前为 GitHub Pages 展示模式，真实 AI 后端未连接。' : gateway.message || '等待调用'}`],
      ['Provider / Model', `${gateway.provider || Store.state.settings.provider} / ${gateway.model || Store.state.settings.model}`],
      ['Streaming', '🟢 已启用'],
      ['Mock fallback', displayMode || Store.state.settings.accessMode === 'api' ? '🟡 关闭' : '🟢 开启'],
      ['Demo Mode', displayMode || Store.state.settings.accessMode === 'local' ? '🟢 开启' : '🟡 关闭'],
      ...
      ['当前模式说明', displayMode ? '当前为 GitHub Pages 展示模式，真实 AI 后端未连接。' : `${Store.state.settings.accessMode === 'local' ? 'Mock 本地演示' : Store.state.settings.accessMode === 'api' ? 'API 模式' : 'Hybrid 模式'}`]
    ];
  ...
  aihistory() {
    ...
    return `${this.pageHead('AI调用历史', '记录时间、模块、Provider、Model、requestId、成功、Mock、耗时、错误原因和 Token 用量，不记录 API Key。', `<button class="secondary-btn" data-action="aihistory-export">${icon('download')}导出TXT</button><button class="secondary-btn" data-action="aihistory-clear">${icon('trash')}清空历史</button><button class="primary-btn" data-action="aihistory-refresh">${icon('refresh')}刷新历史</button>`)}<section class="panel"><div class="panel-head"><div><h3>最近 20 条记录</h3></div><span class="badge">${items.length}</span></div><div class="panel-body">${items.length ? `<div class="table-wrap"><table class="data-table"><thead><tr><th>时间</th><th>requestId</th><th>模块</th><th>Provider</th><th>Model</th><th>成功</th><th>Mock</th><th>耗时</th><th>Prompt</th><th>Completion</th><th>Total</th><th>HTTP</th><th>错误原因</th><th>Raw Error</th></tr></thead><tbody>${items.map(item => `<tr><td>${Utils.formatDate(item.time, true)}</td><td>${Utils.escape(item.requestId || '无')}</td><td>${Utils.escape(item.module || '')}</td><td>${Utils.escape(item.provider || '')}</td><td>${Utils.escape(item.model || '')}</td><td>${item.success ? '是' : '否'}</td><td>${item.mock ? '是' : '否'}</td><td>${item.duration || 0} ms</td><td>${item.promptTokens ?? item.inputTokens ?? '未返回'}</td><td>${item.completionTokens ?? item.outputTokens ?? '未返回'}</td><td>${item.totalTokens ?? '未返回'}</td><td>${item.httpStatus ?? '无'}</td><td>${Utils.escape(item.error || '无')}</td><td>${Utils.escape(item.rawError || '无')}</td></tr>`).join('')}</tbody></table></div>` : this.result('', '暂无 AI 调用历史')}</div></section></div>`;
  },
  datamask() {
  ...
  genericWorkspace(route) {
    ...
    if (route === 'quality') return this.qualityWorkspace(route, ws);
  ...
  about() {
    return `${this.pageHead('项目介绍页', 'Industrial AI OS 的定位、架构、核心能力、适用岗位与后续计划。', `<button class="secondary-btn" data-route="home">${icon('home')}返回首页</button><button class="primary-btn" data-action="demo-flow">${icon('play')}快捷体验</button>`)}<div class="workbench"><div class="stack"><section class="panel"><div class="panel-head"><div><h3>项目定位</h3></div><span class="status-pill success">v1.3 实用版</span></div><div class="panel-body">${this.result('面向制造业与企业办公的 AI 操作系统，覆盖发货单、Excel、Word、PDF、OCR、SQL、AI 写作、Agent、工作流与学习中心，适合真实演示、现场操作和简历投递。', '项目定位', true)}</div></section><section class="panel"><div class="panel-head"><div><h3>核心功能</h3></div></div><div class="panel-body">${this.result('AI聊天、Word助手、Excel助手、PDF助手、OCR识别、AI写作、SQL助手、AI Agent、工作流、Agentic RL 学习中心。', '核心功能', true)}</div></section><section class="panel"><div class="panel-head"><div><h3>技术架构</h3></div></div><div class="panel-body">${this.result('前端：HTML / CSS / JavaScript\n数据：本地存储 + 可选后端同步\nAI：OpenAI 兼容 API\n部署：GitHub Pages + Vercel\n适配：手机 / 平板 / 电脑浏览器。', '技术架构', true)}</div></section></div><div class="stack"><section class="panel"><div class="panel-head"><div><h3>适用岗位</h3></div></div><div class="panel-body">${this.result('制造业文员、生产计划、PMC、采购、销售、外贸、数据处理、企业办公自动化、AI 应用实施、产品演示。', '适用岗位', true)}</div></section><section class="panel"><div class="panel-head"><div><h3>当前进度</h3></div></div><div class="panel-body">${this.result('v1.3 实用版：核心模块已可直接操作，重点围绕 AI 聊天、OCR、PDF、Excel、生产计划、数据校验与任务中心等真实工作链路。', '当前进度', true)}</div></section><section class="panel"><div class="panel-head"><div><h3>后续计划</h3></div></div><div class="panel-body">${this.result('继续补强真实 AI、文件解析、Agent 联动、知识库问答和多设备同步，逐步升级为可交付版本。', '后续计划', true)}</div></section><section class="panel"><div class="panel-head"><div><h3>链接信息</h3></div></div><div class="panel-body"><div class="address-grid"><div class="address-card"><b>GitHub 地址</b><small>${Utils.escape(window.location.origin || '用户部署后填写')}</small></div><div class="address-card"><b>在线地址</b><small>${Utils.escape(Store.state.settings.githubPagesUrl || '用户部署后填写')}</small></div></div></div></section></div></div>`;
  },
  ...
  qualityWorkspace(route, ws) {
    const moduleOptions = ['AI Chat', 'OCR', 'Excel', 'CSV', 'PDF', 'Word', 'PPT', 'SQL', 'MES', 'ERP', 'BOM', '生产计划', '企业办公', '数据管理', 'Tool Center', 'Agent Runtime'];
    return `${this.pageHead('质量助手', '统一检测空值、重复、字段缺失、格式错误、数量金额日期异常、Mock 误用和 AI 不确定内容，支持修复建议、人工确认和导出报告。', `<button class="secondary-btn" data-action="quality-export">${icon('download')}导出报告</button><button class="secondary-btn" data-action="quality-fix">${icon('sparkles')}生成修复建议</button><button class="primary-btn" data-action="quality-check">${icon('check')}开始检测</button>`)}<div class="workbench"><div class="stack"><section class="panel"><div class="panel-head"><div><h3>检测输入</h3></div><span class="status-pill">${Utils.escape(ws.selected || 'AI Chat')}</span></div><div class="panel-body"><div class="field"><label>目标模块</label><select class="select" data-ws-field="selected" data-module="${route}">${moduleOptions.map(item => `<option ${ws.selected === item ? 'selected' : ''}>${item}</option>`).join('')}</select></div><div class="field"><label>待检测内容</label><textarea class="textarea tall" data-ws-field="prompt" data-module="${route}" placeholder="粘贴 AI 聊天内容、OCR 结果、Excel 文本、PDF 摘要、SQL 语句或业务说明">${Utils.escape(ws.prompt || '')}</textarea></div><div class="button-row"><button class="secondary-btn" data-action="quality-fix">${icon('sparkles')}生成修复建议</button><button class="secondary-btn" data-action="quality-export">${icon('download')}导出报告</button><button class="primary-btn" data-action="quality-check">${icon('check')}开始检测</button></div><div class="privacy-note">${icon('shield')}<span>高风险修复会在前端明确提示，必要时进入 Human Approval；不会静默修改真实数据。</span></div></div></section></div><div class="stack"><section class="panel"><div class="panel-head"><div><h3>检测结果</h3></div><span class="badge">${(ws.result || '').length}</span></div><div class="panel-body">${this.result(ws.result || '', '检测结果、风险评级、修复建议和报告会显示在这里', true)}</div></section><section class="panel"><div class="panel-head"><div><h3>前后对比</h3></div><span class="badge">${(ws.before || '').length + (ws.after || '').length}</span></div><div class="panel-body"><div class="field"><label>修复前</label><textarea class="textarea" data-ws-field="before" data-module="${route}" placeholder="检测前内容">${Utils.escape(ws.before || '')}</textarea></div><div class="field"><label>修复后</label><textarea class="textarea" data-ws-field="after" data-module="${route}" placeholder="建议修复后内容">${Utils.escape(ws.after || '')}</textarea></div></div></section></div></div>`;
  },
  ...
  costWorkspace(route, ws) {
    return `${this.pageHead('成本核算助手', '根据数量、材料费、工时成本、加工费和报价金额计算总成本、利润与利润率，并导出分析表。总成本为自动计算结果，不建议手动填写。', `<button class="secondary-btn" data-action="workspace-export" data-module="${route}">${icon('download')}导出分析</button><button class="primary-btn" data-action="cost-calc">${icon('chart')}计算成本</button>`)}<div class="workbench"><div class="stack"><section class="panel"><div class="panel-head"><div><h3>成本输入</h3></div><span class="status-pill">${ws.costStatus || '支持中文字段与自动计算'}</span></div><div class="panel-body"><div class="field-row"><div class="field"><label>数量</label><input class="input" id="costQuantity" type="number" min="0" step="1" data-ws-field="quantity" data-module="${route}" value="${ws.quantity ?? ''}" placeholder="500"></div><div class="field"><label>材料费</label><input class="input" id="costMaterialFee" type="number" min="0" step="0.01" data-ws-field="materialFee" data-module="${route}" value="${ws.materialFee ?? ''}" placeholder="500.00"></div></div><div class="field-row"><div class="field"><label>工时成本</label><input class="input" id="costLaborCost" type="number" min="0" step="0.01" data-ws-field="laborCost" data-module="${route}" value="${ws.laborCost ?? ''}" placeholder="300.00"></div><div class="field"><label>加工费</label><input class="input" id="costProcessFee" type="number" min="0" step="0.01" data-ws-field="processFee" data-module="${route}" value="${ws.processFee ?? ''}" placeholder="1000.00"></div></div><div class="field-row"><div class="field"><label>报价金额</label><input class="input" id="costQuoteAmount" type="number" min="0" step="0.01" data-ws-field="quoteAmount" data-module="${route}" value="${ws.quoteAmount ?? ''}" placeholder="5000.00"></div><div class="field"><label>单价（可选）</label><input class="input" id="costUnitPrice" type="number" min="0" step="0.01" data-ws-field="unitPrice" data-module="${route}" value="${ws.unitPrice ?? ''}" placeholder="如需显示数量×单价成本"></div></div><div class="field"><label>补充说明 / 粘贴字段</label><textarea class="textarea" data-ws-field="prompt" data-module="${route}" placeholder="可粘贴：数量=500\n材料费=500\n工时成本=300\n加工费=1000\n报价金额=5000">${Utils.escape(ws.prompt || '')}</textarea></div><div class="button-row"><button class="secondary-btn" data-action="workspace-save" data-module="${route}">${icon('check')}保存参数</button><button class="primary-btn" data-action="cost-calc">${icon('play')}开始计算</button></div><div class="privacy-note">${icon('shield')}<span>空值按 0 处理；总成本、利润和利润率为自动计算结果，保存参数后会保留当前输入。</span></div></div></section></div><div class="stack"><section class="panel"><div class="panel-head"><div><h3>成本分析结果</h3></div></div><div class="panel-body">${this.result(ws.result, '计算后显示总成本、报价、利润率、风险提示与分析表', true)}</div></section></div></div>`;
  },

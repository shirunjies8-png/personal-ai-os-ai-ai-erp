...
> 文档状态：阶段 2 客户与 RFQ 已完成，阶段 3 尚未开始
...
阶段 1 完成现状盘点、数据模型、迁移策略、API、页面、状态机、编号和兼容方案设计。阶段 2 已按该设计增量实现客户、联系人、项目和 RFQ 闭环；没有开始图纸、CAD、BOM、工艺路线或后续制造阶段，也没有修改 OCR 核心架构或接入 DeepSeek。
...
- [x] 阶段 2 客户与 RFQ 实现（客户、联系人、项目、RFQ、需求、风险、跟进、评审和转报价）。

## 16. 阶段 2 完成范围

- 使用兼容迁移新增客户、联系人、项目、RFQ、需求、风险和跟进实体，并增量扩展通用审计字段；迁移具备版本、校验和事务保护，不删除旧数据。
- 客户、联系人、项目和 RFQ 均支持服务端 SQLite 持久化、企业隔离、搜索、详情、编辑和受控软删除。
- RFQ 必填项、风险等级、阻断项、下一步和状态推进均由确定性代码计算；服务端评审和管理员放行复用现有权限、审批、任务与日志底座。
- RFQ 支持从草稿提交评审、补充资料、可报价、报价中、已报价、洽谈、成交和失效的受控流转，并可把已放行且无阻断项的数据转入现有报价工作区。
- 现有企业工作区询盘只允许显式、幂等导入；导入内容不会自动审批，也不会被伪装为已完成评审。
- GitHub Pages 和后端不可用场景保留 localStorage 演示降级，页面明确提示不会产生服务端保存、审批或正式状态变更。
- 已通过阶段 2 单元/API/SQLite/企业隔离、刷新恢复、独立浏览器读取和 390×844 响应式验证；完整 `verify` 在固定所有子进程使用 Node 22 后通过。下述 OCR 环境问题仍保留为 open，未删除或跳过该门禁。

## 17. 已知问题：ENV-OCR-E2E-001

- 现象：自动化浏览器 OCR 流程在生成测试图片并启动识别后，Tesseract 在 60 秒轮询窗口内持续显示 `initializing tesseract`，既未返回文字也未进入明确不可用状态。
- 影响：该问题曾导致 `npm run verify` 的 OCR E2E 项失败；2026-07-17 阶段 1 和阶段 2 的最新完整验证均通过，但根因尚未修复，因此仍按可间歇复现的环境问题跟踪。它不影响已独立验证的公网 HTTPS API、SQLite CRUD、企业隔离、CORS、阶段 2 普通页面和手机端链路。

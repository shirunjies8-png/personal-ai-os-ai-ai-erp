...
## 真实领料临时审批模型（Phase C.2）

- 当前没有 Employee Master；高风险领料与安全库存覆盖仅复用已认证 `users.role` 的 `admin` 权限。
- 申请人不得审批自己的领料申请；审批事实仅来自 Validator JSON、审批 DTO 与 SQLite 记录，不由 LLM 生成或改写。
- 该临时模型不等同于岗位、仓库、工序或生产订单授权体系，后续主数据阶段再替换。
...
- 2026-07-27 完整浏览器 E2E 发现并修复报价草稿复制后审计视图未刷新：复制成功现在会持久化、刷新审计记录并显示成功提示；`npm run verify` 已覆盖审批 → 保存 → 复制 → 审计链路。
- 2026-07-27 本地真实浏览器在邮箱缺失时仍显示“请输入邮箱和密码”，且控制台 `Errors: 0`；该已处理输入校验不再伪装为运行时异常。

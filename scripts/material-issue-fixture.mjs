...
export async function createMaterialIssueFixture() {
  ...
  const requesterB = { id: 'fixture-requester-b', email: 'requester-b.fixture@example.test', password: 'Requester-B-Only-123!', role: '操作员' };
  ...
  for (const user of [requester, requesterB, approver]) {
  ...
  const rows = [
    ...
    ['fixture-concurrent', 'FIX-CONCURRENT', 'Concurrent inventory', 50, 0]
  ];
  ...
  db.close();
  return { dir, dbPath, enterpriseId, requester, requesterB, approver };
}
